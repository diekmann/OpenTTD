...
TEST_CASE("MakeTile - MakeRailDepot")
{
	...
	REQUIRE(GB(t.m5(), 6, 2) == 0b11); // not free, but hard-coded to 1s.
	//REQUIRE(t.m6() == 0); FIXME: not cleared.
	REQUIRE(t.m7() == 0);
	REQUIRE(GB(t.m8(), 6, 10) == 0);
}
...
TEST_CASE("MakeTile - MakeRoadDepot")
{
	...
	REQUIRE(GB(t.m5(), 6, 2) == 0b10); // not free, but hard-coded to 0b10
	//REQUIRE(GB(t.m6(), 0, 3) == 0); FIXME: not cleared.
	//REQUIRE(GB(t.m6(), 6, 2) == 0); FIXME: not cleared.
	REQUIRE(GB(t.m7(), 6, 2) == 0);
...
TEST_CASE("MakeTile - MakeTree")
{
	...
	MakeTree(t, TREE_INVALID, tree_count, TreeGrowthStage::Dead, TreeGround::TREE_GROUND_ROUGH_SNOW, 3);

	CHECK(GetTreeCount(t) == tree_count + 1);
	CHECK(GetTreeGrowth(t) == TreeGrowthStage::Dead);
	CHECK(GetTreeGround(t) == TreeGround::TREE_GROUND_ROUGH_SNOW);
...
TEST_CASE("MakeTile - MakeRailStation")
{
	...
	MakeRailStation(t, OWNER_NONE, sid, AXIS_Y, section, RAILTYPE_MAGLEV);
...
TEST_CASE("MakeTile - MakeOilrig")
{
	...
	const StationID sid = StationID(42); // randomly chosen.
	MakeOilrig(t, sid, WATER_CLASS_INVALID);
...
TEST_CASE("MakeTile - MakeShipDepot")
{
	...
	const DepotID depot_id = DepotID(42); // randomly chosen.
	MakeShipDepot(t, OWNER_NONE, depot_id, DEPOT_PART_SOUTH, AXIS_Y, WATER_CLASS_INVALID);
	...
	REQUIRE(t.m4() == 0);
	REQUIRE(GB(t.m5(), 2, 2) == 0);
	REQUIRE(GB(t.m5(), 4, 2) == 0b11); // hard-coded to 1s
	//REQUIRE(t.m6() == 0); FIXME: not cleared.
	REQUIRE(t.m7() == 0);
	//REQUIRE(t.m8() == 0); FIXME: not cleared.
}
